...
- **Понад бриф**: порожній стан (empty state) для списку вакансій — коли пошук і фільтр за категорією не дають результатів, замість пустого місця показується повідомлення й кнопка "Скинути фільтри", що одночасно скидає і пошук, і вибрану категорію.
- **Понад бриф**: обране — кнопка-сердечко на картці вакансії; список id зберігається в `localStorage` через `FavoritesContext`/`FavoritesProvider`, лічильник обраних — у Header.
